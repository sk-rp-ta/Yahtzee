...
void Bot::set_Aggression(int x)
{
	Aggression = x;
}

int Bot::get_Aggression()
{
	return Aggression;
}
...
int Bot::place(vector<Player> &players, vector<Bot> &bots, int number_of_players)
{
	...
	for (auto i : players)
	{
		if (get_NumberOfPoints() > i.get_NumberOfPoints()) _place++;
	}
	for (auto i : bots)
	{
		if (get_NumberOfPoints() > i.get_NumberOfPoints()) _place++;
...
bool Bot::do_roll(vector<Player> &players, vector<Bot> &bots, int number_of_players)
{
	...
	double risk = (get_TurnPoints() / 50.0); //actual number of points divided by maximum possible to get

	switch (Aggression)
	{
	case 1:
		return risk < (0.50 + 0.05 * (place(players, bots, number_of_players) - 1)); //lowest posiible value is 6 1+1+1+1+2 so (6/50+1)/2=0.56

	case 2:
		return risk < (0.55 + 0.075 * (place(players, bots, number_of_players) - 1)); // additional value is representation of playing more risky when being on lower places

	case 3:
		return risk < (0.60 + 0.1 * (place(players, bots, number_of_players) - 1));
...
void Bot::b_roll()
{
	...
	bool roll_1 = false, roll_2 = false, roll_3 = false;
	...
	for (int i = 0; i < 5; i++)
	{
		NumberOfValues[get_RollADice(i)] += 1;
		sum += (get_RollADice(i));
	}
	...
	if ( s_straight1 || s_straight2 || s_straight3 )
	{
		for (int i = 0; i < 5; i++)
		{
			if (NumberOfValues[get_RollADice(i)] == 2) value = i;
		}
		if (value == 0 && s_straight1) //when 1 2 3 4 and 6
		{
			for (int i = 0; i < 5; i++)
			{
				if (get_RollADice(i) == 6) value = i;
			}
		}
	...
	else if ((close_ss_1 || close_ss_2 ||close_ss_3 || close_ss_4) && doublevalue == 0)
	{
		if (close_ss_1)
		{
			for (int i = 0; i < 5; i++)
			{
				if (get_RollADice(i) > 3)
				{
					temp << i+1;
					set_RollADice(doublevalue);
				}
			}
		}
		if (close_ss_2)
		{
			for (int i = 0; i < 5; i++)
			{
				if(get_RollADice(i) < 2 || get_RollADice(i)>4)
				{
					temp << i+1;
					set_RollADice(doublevalue);
				}
		...
		if (close_ss_3)
		{
			for (int i = 0; i < 5; i++)
			{
				if (get_RollADice(i) < 3 || get_RollADice(i) > 5)
				{
					temp << i+1;
					set_RollADice(doublevalue);
				}
	...
	else if (roll_2)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			if (get_RollADice(i) == first || get_RollADice(i)==second)
			{
				temp << i+1;
				set_RollADice(i);
			}
